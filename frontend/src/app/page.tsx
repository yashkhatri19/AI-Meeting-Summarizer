...
function Dashboard() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "Binary sync dropped.");
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      setMessages([...updatedMessagesWithUser, { sender: "ai" as const, text: data.reply || "Agent offline." }]);
    } catch (err) {
        ...
        <div className="md:col-span-2 lg:col-span-3 flex flex-col gap-4 h-full max-h-full overflow-hidden">
          <div className="bg-slate-900/20 border-2 border-dashed border-slate-800 rounded-2xl p-5 flex flex-col items-center justify-center min-h-[140px] relative shrink-0">
            ...
            {!file ? (
              ...
              <div className="text-center flex flex-col items-center gap-2 z-20">
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); }} className="text-[10px] text-rose-400 underline cursor-pointer hover:text-rose-300">Cancel</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900/20 border border-slate-800/60 rounded-2xl p-5 flex flex-col flex-1 h-full min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto text-xs text-slate-300 bg-slate-950/40 p-4 rounded-xl border border-slate-800/60 leading-relaxed font-mono whitespace-pre-wrap min-h-[150px]">
              {transcript ? transcript : "Context channel inactive."}
            </div>
          </div>
        </div>
